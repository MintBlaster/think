...
RenderWindow::RenderWindow(const char *title, int width, int height)
    : window_(nullptr), glContext_(nullptr), shaderProgram(0), VAO(0), VBO(0),
      EBO(0), screenWidth(width), screenHeight(height), texture(0) {
...
void RenderWindow::loadShaders(const char* vertexShaderPath, const char* fragShaderPath) {
  ...
  shaderProgram = createShaderProgram((BASE_RES_DIR + vertexShaderPath).c_str(), (BASE_RES_DIR + fragShaderPath).c_str());
}
...
void RenderWindow::display() const { SDL_GL_SwapWindow(window_); }
...
void RenderWindow::renderTexture(GLuint textureID, float x, float y, float width, float height) const {
  ...
  // Create model matrix
  glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(x, y, 0.0f)) * glm::scale(glm::mat4(1.0f), glm::vec3(width, height, 1.0f));
