...
struct Vector4 {
    ...
    [[nodiscard]] Vector4 addX(const float value) const {
        return {x + value, y, z, w};
    }
    [[nodiscard]] Vector4 addY(const float value) const {
        return {x, y + value, z, w};
    }
    [[nodiscard]] Vector4 addZ(const float value) const {
        return {x, y, z + value, w};
    }
    [[nodiscard]] Vector4 addW(const float value) const {
...
struct Matrix4 {
    ...
    Matrix4 operator*(const Matrix4 &other) const {
        ...
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                result[i][j] = rows[i].x * other.rows[0][j] +
